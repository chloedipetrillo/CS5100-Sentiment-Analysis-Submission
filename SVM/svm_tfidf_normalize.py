...
class TFIDFNormalize():
    '''
    Class TFIDFNormalize is a class that vectorizes data using the
        term frequency - inverse document frequency technique and normalizes using 
        L2 normalization.
    '''
    def __init__(self):
    ...
    def transform(self, tfidf_and_matrix):
        ...
        Returns:
            tfidf: normalized NumPy array with tfidf values
        '''
...
def run_svm(filename, kernel, test_size, data_size):
    ...
    test_predictions = svm.predict(test_vectors)
